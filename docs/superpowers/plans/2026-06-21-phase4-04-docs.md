...
services:
  media-scan-monitor:
    ...
    ports:
      - "8080:8080"
    volumes:
      # Persistent config: app.db (settings) and secret.key (Fernet encryption key).
      # Must be writable by UID 1000 — run: chown -R 1000:1000 ./config
      - ./config:/config
      # *** LOCAL storage ONLY. inotify does not work over NFS/SMB. ***
      # Bind-mount media from a local filesystem. Replace with your actual path.
      - /path/to/your/local/media:/data/media:ro
    environment:
      TZ: Europe/London
      # First-run password bootstrap (never overwrites a password set in the UI).
      # Use a Docker secret file for better security:
      # MSM_PASSWORD_FILE: /run/secrets/msm_password
      MSM_PASSWORD: "change-me-on-first-run"

  # OPTIONAL: raise the kernel inotify watch limit.
  # Enable with: docker compose --profile init-watches up
  # WARNING: this sidecar is PRIVILEGED and writes a HOST-WIDE kernel value
  # (fs.inotify.max_user_watches). It affects every process on the host,
  # not just this container. Use the host sysctl.d route (see docs) instead
  # if you prefer not to run privileged containers.
  init-watches:
    image: busybox
...
Navigate to `http://<your-host>:8080`. On first run (if `MSM_PASSWORD` is set) the app
bootstraps that password; otherwise the setup screen prompts you to create one. Log in.
...
for normal operation. The only bootstrap input the container reads from the environment is
the first-run password; everything else (servers, folders, file types, debounce policy,
inotify gate) is managed through the UI and stored in `app.db`.
...
| `MSM_SECRET_KEY_FILE` | `/config/secret.key` | Path to the Fernet key file used to encrypt server secrets at rest (auto-created if absent). |
| `MSM_SECRET_KEY` | — | Fernet key provided inline (alternative to the file; file takes precedence). |
...
After the existing (now resolved) migrations item and the inotify-resilience item, add a new
subsection. The full Phase 4 section should read as follows after both edits (replace the entire
`## Phase 4 — observability & image` section with this):
